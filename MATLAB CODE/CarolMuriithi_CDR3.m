...
%% select for nonCDR and CDR regions and assign features
...
% x_train structure: A row corresponds to an amino acid. 
% The columns are the features (...) for CDRH3 region. 


for j = 1:n_train % loop of rows of data

     if data.H92(j) == double('C') && data.H93(j) == double('A') && data.H94(j) == double('R')
        %%if data.H103(j) == double('W') && data.H104(j) == double('G') && (data.H105(j) == double('Q') || data.H105(j) == double('T') && data.H106(j) == double('G'))
